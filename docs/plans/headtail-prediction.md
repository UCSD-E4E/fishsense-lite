...
ones, using `fishsense_core.fish` gated on the image's validated laser dot.
...
### 0.2b Coverage is capped by detector recall, and that is now movable
...
phenomenon: the fish is too small once a 4014×3016 frame is squeezed to
1058×795 for inference.

That is fixable without labels. Measured on 80 frames
([fish-detection-labeling.md §0.4](fish-detection-labeling.md)): tiling lifts
detection of the laser-designated fish from 50.0% to 63.8% on the same Fishial
weights, and SAM3 tiled reaches 72.5%. Whatever this stage's populate does, its
coverage rides on that number — so build the stage against today's 57.7% and
expect it to improve underneath, rather than gating the design on it.

**Note the landscape-only bug** (`fish-detection-labeling.md` §0.5): tiles must
be landscape until the fishsense-core fix ships in a released wheel.

### 0.3 The laser gate is load-bearing

It picked a different fish than `inference_single` would have on **65 of 323**
predictions (20%), and abstained on **95 images** (17%) where no instance sat
under the dot. Because `FishSegmentation.inference` returns an instance
*label map* rather than a binary mask, the gate is one array lookup:
`labels[round(y), round(x)]`.
...
### 1.2 CPU queue, not GPU

`fishsense_core.fish` is in the **base wheel** — not behind
`[laser-detector]` or `[slate]` — with ONNX Runtime statically linked and the
weights embedded in the `.so`. There is no checkpoint to bake into the image,
unlike `predict_laser_image`'s `/e4efs/models/run3_epoch_021.pt`. Measured at
~2 s/frame on CPU. So it registers in the `cpu` role on
`fishsense_data_processing_queue`, and never competes for an NRP GPU.
...
PredictHeadtailImagesParentWorkflow          (api-worker, fishsense_api_queue, hourly +32)
  ...
  3. ensure_data_worker_running_activity                (CPU deployment)
  4. start_child_workflow PredictHeadtailImagesWorkflow (fishsense_data_processing_queue)
  5. persist_headtail_predictions_activity(results) -> int
```

No `stage_raw` and no `cleanup_raw` (§1.1) — this parent is structurally
closer to stages 13/14 than to the preprocess parents.
...
### 4.3 `predict_headtail_image` (data-worker, `cpu` role)

```
download_processed_jpeg(folder, checksum)      # new; see §1.1
  -> cv2.imdecode (BGR — never convert)
  -> FishSegmentation.inference(bgr)           # instance label map
  -> instance under the first laser point that hits  (else abstain)
...
and an unguarded load gives every thread its own copy of the session.
...
11. Pure logic: laser inside instance 2 of a synthetic 3-instance label map →
    picks 2; laser on background → `laser_off_all_fish`; empty map →
    `no_detections`; `find_head_tail_img` raising → `headtail_failed`.
12. Pure logic: `silhouette_ratio` = `mask_area_px / pred_len**2`, and the
    band check at the §0.5 boundaries.
13. In-process Temporal workflow contract test for the fan-out.
14. Integration (`-m integration`) against a real JPEG fixture.

**Populate**
15. Prediction present → task carries two `kp-1` keypoints labelled `Snout`
    and `Fork` at the right percentages.
16. Prediction absent → image is **not** populated (prediction-gated).
17. `rejected_low_confidence` → task seeded with **no** predictions, not
    skipped.
18. Re-run against a project that already has the task → no duplicate import.

**Tripwires**
19. `predict_headtail_image` imports no NAS client.
20. `test_schedule_registration` — +32 and +34 present, preprocess parent no
