"""Lower a dive's stage 2 redraw flags once its JPEGs have been rewritten.

`needs_reprocess` is the one term in the stage 2 cohort predicate that does not
...
Called by the stage 2 parent after its data-worker child completes, and only on
...
@activity.defn
async def clear_species_reprocess_flags_activity(dive_id: int) -> int:
    """Clear `needs_reprocess` on every species label of a dive's canonical
    images. Returns the number of rows cleared."""
    async with get_fs_client() as fs:
        cleared = await fs.labels.clear_species_needs_reprocess(dive_id)

    activity.logger.info(
        "cleared species reprocess flags dive_id=%d rows=%d", dive_id, cleared
    )
    return cleared
