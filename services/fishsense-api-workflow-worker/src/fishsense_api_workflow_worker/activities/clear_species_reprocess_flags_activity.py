...
@activity.defn
async def clear_species_reprocess_flags_activity(
    ...
    async with get_fs_client() as fs:
        cleared = await fs.labels.clear_species_needs_reprocess(
            payload.dive_id, payload.checksums
        )

    activity.logger.info(
        "cleared species reprocess flags dive_id=%d rows=%d scope=%s",
        payload.dive_id,
        cleared,
