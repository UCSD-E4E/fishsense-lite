...
@activity.defn
async def clear_headtail_reprocess_flags_activity(
    ...
    async with get_fs_client() as fs:
        cleared = await fs.labels.clear_headtail_needs_reprocess(
            payload.dive_id, payload.checksums
        )

    activity.logger.info(
        "cleared headtail reprocess flags dive_id=%d rows=%d scope=%s",
        payload.dive_id,
        cleared,
