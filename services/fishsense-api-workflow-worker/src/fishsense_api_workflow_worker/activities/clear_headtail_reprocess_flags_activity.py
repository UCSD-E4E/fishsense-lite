"""Lower a dive's stage 5.1 redraw flags once its JPEGs have been rewritten.

`needs_reprocess` is the one term in the stage 5.1 cohort predicate that does not
...
Called by the stage 5.1 parent after its data-worker child completes, and only on
...
@activity.defn
async def clear_headtail_reprocess_flags_activity(dive_id: int) -> int:
    """Clear `needs_reprocess` on every head/tail label of a dive's canonical
    images. Returns the number of rows cleared."""
    async with get_fs_client() as fs:
        cleared = await fs.labels.clear_headtail_needs_reprocess(dive_id)

    activity.logger.info(
        "cleared headtail reprocess flags dive_id=%d rows=%d", dive_id, cleared
    )
    return cleared
