...
@activity.defn
async def clear_slate_reprocess_flags_activity(
    ...
    async with get_fs_client() as fs:
        cleared = await fs.labels.clear_dive_slate_needs_reprocess(
            payload.dive_id, payload.checksums
        )

    activity.logger.info(
        "cleared slate reprocess flags dive_id=%d rows=%d scope=%s",
        payload.dive_id,
        cleared,
