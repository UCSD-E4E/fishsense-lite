...
"""Workflow contract test for PreprocessHeadtailImagesParentWorkflow."""
...
def _make_stubs(
    ...
    @activity.defn(name="select_next_high_priority_dive_for_headtail_preprocessing_activity")
    async def stub_select() -> Optional[int]:
        return selector_result

    @activity.defn(name="resolve_headtail_preprocess_inputs_activity")
    async def stub_resolve(dive_id: int) -> PreprocessHeadtailImagesInput:
...
async def test_dispatches_child_with_deterministic_id():
    ...
    child_id, child_dive_id, checksums = child_runs[0]
...
async def test_returns_none_when_no_dive():
    ...
    async with await WorkflowEnvironment.start_time_skipping() as env:
        async with Worker(
            ...
            result = await env.client.execute_workflow(
                PreprocessHeadtailImagesParentWorkflow.run,
                id=f"test-stage5-1-parent-none-{uuid.uuid4()}",
                task_queue="test-stage5-1-parent-none",
            )
...
async def test_skips_child_when_no_image_checksums():
    ...
    async with await WorkflowEnvironment.start_time_skipping() as env:
        async with Worker(
            ...
            result = await env.client.execute_workflow(
                PreprocessHeadtailImagesParentWorkflow.run,
                id=f"test-stage5-1-parent-empty-{uuid.uuid4()}",
                task_queue="test-stage5-1-parent-empty",
            )
