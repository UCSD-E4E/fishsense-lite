...
def test_build_task_emits_dual_image_and_img_keys(monkeypatch):
    ...
    # Both keys, both the same URL. Asserted per-key rather than as an exact
    # dict so the capture-order fields (`taken`, `image_id`) can ride along
    # without this pin — which is about dual emission — failing on key count.
