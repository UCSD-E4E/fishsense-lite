"""Shared upsert for the model-prediction tables.

`LaserPrediction` and `SlatePrediction` are written by their predict stages
with identical semantics: one row per image, upserted on `image_id`, stamped
server-side. Their PUT handlers were byte-identical apart from the model name —
which is exactly the duplication `duplicate-code` cannot see, because
`LaserPrediction.image_id == image_id` and `SlatePrediction.image_id ==
image_id` are different strings. CLAUDE.md names this shape and the four
`put_*_label` handlers that had it; a green pylint says nothing about it.

Two handlers is a thin case for extraction on its own. It is worth it here
because the alternative is writing the same subtle ordering twice, and because
a third caller returns with the head/tail predict stage.

Both behaviours are load-bearing, and each was a bug before it was a rule:
...
The stamp must come **after** the id resolution. Assigning it earlier lets
`merge` load the existing row and overwrite it, which is silent — the column
stays NULL and nothing fails.
"""
...
async def upsert_prediction(
    ...
    # After the id resolution, so it survives the merge.
