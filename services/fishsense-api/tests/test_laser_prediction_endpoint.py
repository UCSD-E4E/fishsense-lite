...
async def test_put_stamps_created_at(session):
    """`created_at` is declared on every prediction table and written by nothing.
    ...
    two cases: backfill deliberately skips completed tasks, so a prediction
    written *after* a label's `updated_at` provably never reached a labeler,
    while one written before it is the row that was shown. Without it, "did the
    labeler move the seeded point?" is unanswerable for the whole corpus.
    """
    from fishsense_api.controllers.laser_prediction_controller import put_laser_prediction
    from fishsense_api.models.laser_prediction import LaserPrediction
