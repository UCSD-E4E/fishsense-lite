...
class TestDiveClient:
    ...
    async def test_select_next_for_laser_depth_hits_the_cohort_endpoint(self):
        client = _make_client()
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = 42
        ...
        with patch.object(client, "_get", new_callable=AsyncMock) as mock_get:
            ...
            async with client:
                result = await client.select_next_for_laser_depth()
                assert result == 42
                mock_get.assert_awaited_once_with(
                    "/api/v1/dives/select-next/laser-depth/"
                )
...
class TestSelectNextForLaserAutoAccept:
    """The auto-accept gate's backlog selector."""

    async def test_select_next_for_laser_auto_accept_hits_the_cohort_endpoint(self):
        client = _make_client()
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = 42
        ...
        with patch.object(client, "_get", new_callable=AsyncMock) as mock_get:
            ...
            async with client:
                result = await client.select_next_for_laser_auto_accept()
                assert result == 42
                mock_get.assert_awaited_once_with(
